...
# unigrams <- NGramTokenizer(corp, Weka_control(min=1, max=1))
# bigrams <- NGramTokenizer(corp, Weka_control(min = 2, max = 2, delimiters = " \\r\\n\\t.,;:\"()?!"))
# trigrams <- NGramTokenizer(corp, Weka_control(min = 3, max = 3, delimiters = " \\r\\n\\t.,;:\"()?!"))
# quadgrams <- NGramTokenizer(corp, Weka_control(min = 4, max = 4, delimiters = " \\r\\n\\t.,;:\"()?!"))

###data.frame(text = sapply(corp, as.character), stringsAsFactors = FALSE)
...
library(tm)
library(RWeka)
freqTerms <- findFreqTerms(ngrammatr2, lowfreq = 7000)
